...
// 擬似的なデータフェッチ関数
export function fetchTimeTableDatabyId(id: string): Nebuta | undefined {
  let now = new Date();
  ...
  const nebutas: Nebuta[] = [
    ...
    },
    {
      ...
      image: "sample",
      entryNumber: "sample",
      time_table: initializeTimeTable(shiftArray(points)[8], now, requireTime),
    },
  ];
  return nebutas.find((nebuta) => nebuta.id === id);
}

export function fetchTimeTableData(): Nebuta[] {
  const points: Waypoint[] = [
    { lat: 35.65402334223389, lng: 139.74982448505963 },
    { lat: 35.65485274960821, lng: 139.7451873649693 },
    { lat: 35.65966314254042, lng: 139.74285422297962 },
